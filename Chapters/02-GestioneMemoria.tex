...
\end{minted}
In questo esempio, la modifica del campo \texttt{name} di \texttt{p2} influisce anche \texttt{p1}, poiché entrambi i riferimenti puntano allo stesso oggetto in memoria. Questo comportamento può causare bug sottili e difficili da individuare, specialmente in contesti complessi o concorrenti. In Rust, invece, il trasferimento di ownership impedisce il comportamento appena descritto, poiché una volta che l'ownership è stata trasferita, la variabile originale non può più essere utilizzata.

Il meccanismo di passaggio degli argomenti in Rust è strettamente legato al concetto di ownership. Quando si passa una variabile a una funzione, l'ownership di quella variabile viene trasferita alla funzione, rendendo la variabile originale non più utilizzabile dopo la chiamata.Questo avviene perché, in Rust, il passaggio di un argomento avviene per \textit{move}. Di conseguenza, la funzione riceve l'ownership della variabile e ne diventa pienamente responsabile. Ad esempio, consideriamo il seguente codice:
\begin{minted} [fontsize=\small] {Rust}
    fn main() {
        let s1 = String::from("Hello");
        print_string(s1); 
...
È importante sottolineare che la semantica del \textit{move} in Rust, così come descritta finora, si applica principalmente ai tipi di dati che allocano memoria sull'heap, come \texttt{String}, \texttt{Vec} o strutture complesse. In questi casi, un'assegnazione o il passaggio a una funzione comporta il trasferimento dell'\textit{ownership}, e quindi l'invalidazione del valore originale. Tuttavia, per i tipi semplici e a dimensione fissa nota a compile time, come gli interi (\texttt{i32}, \texttt{u64}), i booleani (\texttt{bool}), i caratteri (\texttt{char}) e i puntatori a funzione, Rust applica una semantica diversa: questi tipi implementano automaticamente il trait \texttt{Copy}. Ciò significa che, in fase di assegnazione o di passaggio come parametro a una funzione, viene eseguita una copia bit a bit del valore, e l'\textit{ownership} non viene trasferita.
...
In sintesi, in Rust:
\begin{itemize}
    \item I tipi che implementano il trait \texttt{Copy} vengono copiati senza invalidare il valore originale.
    \item I tipi che non implementano il trait \texttt{Copy} (come \texttt{String}, \texttt{Vec}) vengono spostati tramite \textit{move}, trasferendo l'\textit{ownership} ed evitando deep copies non necessarie.
\end{itemize}
Questa distinzione riflette chiaramente la filosofia di Rust nel garantire la sicurezza nell'accesso alla memoria:
\begin{itemize}
    \item Per i tipi che possono contenere dati allocati dinamicamente o che possono essere modificati a runtime, Rust usa la semantica del \textit{move}, che impedisce di accedere a un valore dopo che la sua \textit{ownership} è stata trasferita altrove, evitando così potenziali problemi di accesso concorrente o modifiche inattese.
    \item Per i tipi con dimensione fissa e nota a compile time, che risiedono interamente nello \textit{stack} e sono di solito immutabili per definizione (come interi o booleani), Rust utilizza la semantica del \textit{copy}, poiché la copia bit-a-bit è efficiente e non introduce rischi di inconsistenza o accessi errati.
\end{itemize}
