...
\begin{itemize}
    ...
    \item Il compilatore Java non forza l'utilizzo di \texttt{Optional<T>} al posto di \texttt{null}. Java mette a disposizione il costrutto ma non lo impone, lasciando la scelta al programmatore, che può essere più incline a utilizzare \texttt{null} per semplicità.
...
\end{itemize}
Rust, invece, elimina completamente la possibilità di utilizzare \texttt{null}, andando a risolvere molti dei problemi sopra elencati. Tuttavia, il concetto rappresentato da \texttt{null} è sempre utile. Per questo Rust rappresenta l'assenza di valore con un approccio simile a \texttt{Optional<T>} di Java, ossia tramite il tipo \texttt{Option<T>}, definito nella libreria standard come:
\begin{minted}[fontsize=\small]{rust}
    enum Option<T> {
...
\end{minted}
L'idea alla base di \texttt{Option<T>} è analoga a quella di \texttt{Optional<T>}: fornire un modo per rappresentare l'assenza di un valore in modo sicuro e senza ambiguità.

\texttt{Option<T>} è un \texttt{enum} e \texttt{Some(T)} \footnote{In Rust una variante di un \texttt{enum} può contenere uno o più valori, ognuno con il suo tipo. In questo caso \texttt{Some(T)} contiene un valore di tipo \texttt{T}.} e \texttt{None} sono le sue due varianti. La differenza principale rispetto a Java sta nel fatto che in Rust una variabile non può avere valore \texttt{null}. Per rappresentare l'assenza di valore il programmatore è costretto ad utilizzare \texttt{Option<T>}. Ad esempio, utilizzando la classe \texttt{Person} del listato \ref{lst:java-reference}, definiamo il seguente esempio \footnote{Assumiamo che il campo \texttt{name} sia unico per ogni \texttt{Person}.}: 
\begin{listing}[h]
    \begin{minted}[fontsize=\small]{java}
...
\end{minted}
Sembrerebbe che Rust risolva il problema di \texttt{null} rimuovendolo dal linguaggio e obbligando il programmatore a utilizzare \texttt{Option<T>} per rappresentare l'assenza di valore. Tuttavia, così come in Java, la responsabilità di utilizzare correttamente \texttt{Option<T>} ricade sul programmatore. L'uso corretto di \texttt{Optional<T>} in Java e \texttt{Option<T>} in Rust implica:
\begin{itemize}
    \item Produrre un'alternativa valida quando il valore non è presente. 
    \item Consumare il valore se è presente. 
    \item Evitare l'accesso diretto non controllato al valore (ad esempio \texttt{get()} in Java o \texttt{unwrap()} in Rust), poiché ciò sarebbe diverso da accedere a un valore \texttt{null}.
\end{itemize}
...
\begin{comment}
    Un altro meccanismo fondamentale di Rust per garantire la sicurezza della memoria è il concetto di \textit{lifetimes} (durata di vita) di un riferimento. Se non esistesse questo concetto, il compilatore non sarebbe in grado di garantire che un riferimento sia sempre valido durante il suo utilizzo. Questa è una diretta conseguenza del fatto che, una variabile, in Rust, viene automaticamente deallocata quando esce dallo scope. Infatti, il seguente codice non compila:
    \begin{minted}[fontsize=\small]{rust}
        fn main() {
            let r;
            {
    ...
    \end{minted}
    Questo perché \texttt{s} esce dallo scope e viene deallocata, rendendo \texttt{r} un dangling pointer. Il compilatore di Rust non permette questo codice poiché non può garantire che \texttt{r} sia sempre valido durante il suo utilizzo. Java, invece, non permette di avere dangling pointers, poiché il garbage collector non dealloca memoria finché ci sono riferimenti attivi a un oggetto. Quindi, non si può avere un riferimento a una variabile non più valida. Il codice equivalente Java: 
    \begin{minted}[fontsize=\small]{Java}
        public class Main {
            public static void main(String[] args) {
                String r;
                {
                    ...
                    r = s; // r punta a s
                } // s esce dallo scope, ma r è ancora valido
                System.out.println("Il valore è: " + r);
            }
        }
    \end{minted}
    è del tutto valido e non può generare a errori a run-time poiché la stringa \texttt{"ciao"} è ancora raggiungibile tramite il riferimento \texttt{r} e quindi non viene deallocata una volta che \texttt{s} esce dallo scope, come invece avviene in Rust.
\end{comment}
