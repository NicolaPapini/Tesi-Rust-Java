...
\section{Borrowing}
É evidente come l'ownership sia un concetto potente per la gestione della memoria, ma può risultare troppo restrittivo in alcune situazioni come quella riportata nel codice (\ref{lst:ownership-return}). Rust per risolvere questo problema introduce il concetto di \textit{borrowing}, che consente di prendere in prestito un valore senza trasferirne l'ownership, consentendo una maggiore flessibilità. Il \textit{borrowing} è realizzato attraverso il concetto di \textit{riferimento} (reference). Il riferimento in Rust non ha le stesse proprietà di un riferimento in Java: 
\begin{itemize}
    ...
    Un riferimento Java può essere \texttt{null}, il che significa che non punta a nessun oggetto valido in memoria. Questo ha gravi ripercussioni sulla sicurezza del programma, poiché l'accesso a un riferimento \texttt{null} può causare un \texttt{NullPointerException} a run-time. Lo stesso creatore della nozione del riferimento \texttt{null}, Tony Hoare, lo ha definito "billion dollar mistake"\cite{hoare-null-reference}, un errore che ha portato a innumerevoli bug e vulnerabilità nei programmi Java.
...
\end{itemize}
I riferimenti in Rust sono ottenuti utilizzando l'operatore di referenziazione \texttt{\&}. Ad esempio:
\begin{minted}[fontsize=\small]{rust}
    fn main() {
        let s = String::from("Hello");
        print_string(&s); 
