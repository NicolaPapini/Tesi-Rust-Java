...
Un aspetto fondamentale del polimorfismo è la risoluzione dei metodi, ossia il processo tramite cui il sistema determina quale versione specifica di un metodo deve essere eseguita al momento dell'invocazione. Questo processo può avvenire in due modi principali: staticamente (a compile-time) o dinamicamente (a run-time). Rust e Java utilizzano entrambi questi meccanismi, ma in modi diversi a causa delle loro differenze di progettazione e filosofia. In questa sezione viene trattato il funzionamento di una chiamata di metodo in entrambi i linguaggi. In particolare, per quanto riguarda Rust, il termine metodo si riferisce a una funzione definita nel contesto di una \texttt{struct} (o \texttt{enum}), definita all'interno di un blocco \texttt{impl}. Mentre per funzione si intende una funzione indipendente che viene chiamata utilizzando il suo nome seguito dagli argomenti tra parentesi (ad esempio, \texttt{function(arg1, arg2)}).
...
Il caso più interessante avviene quando viene eseguita una chiamata di metodo. Le chiamate di metodo vengono risolte a metodi associati a trait specifici, eseguendo
\begin{itemize}
