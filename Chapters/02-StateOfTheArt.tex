...
\end{minted}
In questo caso, secondo la regola 3, quando la variabile \texttt{s} esce dallo scope, il valore \texttt{"Hello"} viene automaticamente rilasciato dalla memoria. In java questo non accade. La liberazione della memoria avviene attraverso la funzione \texttt{drop} che viene chiamata automaticamente da rust quando la variabile va fuori dallo scope. Dato il seguente codice equivalente in Java:
\begin{minted} [fontsize=\small] {Java}
    {
        String s = "Hello";
    }
...
\begin{figure}[H]
    \label{fig:own1}
    \centering
    \includegraphics[scale = 0.8]{Figures/own1.pdf}
    \caption{Visualizzazione memoria Rust dopo il trasferimento di ownership.}
\end{figure}
In Rust, per variabili il cui valore è contenuto in memoria heap, un'istruzione di copia esegue quindi una \textit{shallow copy} del valore e invalida la variabile originale. Questo comportamento prende il nome di \textit{move}. Rust non esegue mai una \textit{deep copy} di una variabile, a meno che non sia esplicitamente richiesto dal programmatore, quindi l'operazione di copia base è poco costosa in termini di performance.

Questo non è consistente con quello che accade in Java, dove l'assegnazione di un oggetto a un'altra variabile non invalida la variabile originale, ma crea una nuova referenza, uguale all'originale, all'oggetto esistente. In Java, il codice equivalente sarebbe:
\begin{minted} [fontsize=\small] {Java}
    String s1 = "Hello";
    String s2 = s1; 
...
\end{minted}
In questo caso, quindi, entrambe le stringhe verrebbero correttamente stampate. L'approccio di Rust è più restrittivo ma questa è una caratteristica desiderabile perché permette di evitare errori di memoria comuni, come l'accesso a variabili non valide o la modifica di dati condivisi in modo imprevisto. 
...
\end{minted}
Infatti, la modifica del nome di \texttt{p2} influisce anche su \texttt{p1}, poiché entrambi puntano allo stesso oggetto in memoria. Questo può portare a bug difficili da individuare, specialmente in programmi complessi. In Rust, invece, il trasferimento di ownership impedisce il comportamento appena descritto, poiché una volta che l'ownership è stata trasferita, la variabile originale non può più essere utilizzata.
%TODO: inserire un'esempio in cui in java un metodo modifica un oggetto non voluto
